...
int main(int argc, char **argv)
{
    ...
    A  = doubleVector( smax );
    B  = doubleVector( smax );
    stat1 = (MPI_Status *)malloc( windowSize*sizeof(MPI_Status) );
    stat2 = (MPI_Status *)malloc( windowSize*sizeof(MPI_Status) );
    req1  = (MPI_Request *)malloc( windowSize*sizeof(MPI_Request) );
    req2  = (MPI_Request *)malloc( windowSize*sizeof(MPI_Request) );
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "mrate_time-ws_%.3d-np_%.4d.dat", windowSize, nprocs );
        sprintf( file2, "mrate_rate-ws_%.3d-np_%.4d.dat", windowSize, nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
    ...
    if( proc < npairs ){
        ...
        for(j = 0; j < NLOOP; j++){
            ...
            MPI_Waitall( windowSize, req1, stat1 );
        }
        ...
        for(j = 0; j < NLOOP; j++){
            ...
            MPI_Waitall( windowSize, req2, stat2 );
        }
        timeMin = benchTimer() - tStart;
    }
