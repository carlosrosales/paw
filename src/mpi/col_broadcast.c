 ...
 *==============================================================================
 * Test the effective bandwidth of MPI_Bcast for a set of message sizes.
 *============================================================================*/
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "MPI_Bcast", file1[64], file2[64];
    ...
    UsedMem = (double)smax*(double)dblSize;
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "bcast_time-np_%.4d.dat", nprocs );
        sprintf( file2, "bcast_bw-np_%.4d.dat",   nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
    ...
    MPI_Bcast( A, smed, MPI_DOUBLE, 0, MPI_COMM_WORLD );
    // Test is current NLOOP is enough to capture fastest test cases
	MPI_Barrier( MPI_COMM_WORLD );
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        MPI_Bcast( A, smin, MPI_DOUBLE, 0, MPI_COMM_WORLD );
    }
