 ...
 * an attempt at measuring the maximum sustained one-directional bandwidth 
 * of the system.
 *============================================================================*/
...
int main(int argc, char **argv)
{
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "isend_time-np_%.4d.dat", nprocs );
        sprintf( file2, "isend_bw-np_%.4d.dat",   nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
    ...
    if( proc < npairs ){
        for(j = 0; j < NLOOP; j++){
            ...
            MPI_Wait( &req1, &stat1 );
            MPI_Irecv( B, smin, MPI_DOUBLE, partner, tag, MPI_COMM_WORLD, &req1);
            MPI_Wait( &req1, &stat1 );
        }
    }else{
        for(j = 0; j < NLOOP; j++){
            ...
            MPI_Wait( &req2, &stat2 );
            MPI_Isend( A, smin, MPI_DOUBLE, partner, tag, MPI_COMM_WORLD, &req2 );
            MPI_Wait( &req2, &stat2 );
        }
    }
        ...
        for(i = 0; i < NREPS; i++){
            ...
            if( proc < npairs ){
                for(j = 0; j < NLOOP; j++){
                    ...
                    MPI_Wait( &req1, &stat1 );
        	        MPI_Irecv( B, size, MPI_DOUBLE, partner, tag, MPI_COMM_WORLD, &req1 );
                    MPI_Wait( &req1, &stat1 );
                }
            }
            else{
                for(j = 0; j < NLOOP; j++){
                    ...
                    MPI_Wait( &req2, &stat2 );
        	        MPI_Isend( A, size, MPI_DOUBLE, partner, tag, MPI_COMM_WORLD, &req2 );
                    MPI_Wait( &req2, &stat2 );
                }
            }
            // Fctor 0.5 included because we go both ways
