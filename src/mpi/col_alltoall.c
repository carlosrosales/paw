 ...
 *==============================================================================
 * Test the effective bandwidth of MPI_Alltoall for a set of message sizes.
 *============================================================================*/
...
{
    FILE   *fp, *fp2;
    char   testName[32] = "MPI_Alltoall", file1[64], file2[64];
    ...
    double tScale = USEC, bwScale = MB, resize;
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "alltoall_time-np_%.4d.dat", nprocs );
        sprintf( file2, "alltoall_bw-np_%.4d.dat",   nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
        printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
        fprintf( stdout, "\n\t MAX_COL_SIZE reset to %d", smax );
    }
    ...
    MPI_Alltoall( A, smed, MPI_DOUBLE, B, smed, MPI_DOUBLE, MPI_COMM_WORLD );
    // Test is current NLOOP is enough to capture fastest test cases
    MPI_Barrier( MPI_COMM_WORLD );
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        MPI_Alltoall( A, smin, MPI_DOUBLE, 
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for(i = 0; i < NREPS; i++){
            ...
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                MPI_Alltoall( A, size, MPI_DOUBLE, 
        ...
        if( proc == 0 ){
            ...
            msgBytes  = (double)(size*nprocs*dblSize);
