 ...
 *==============================================================================
 * Test the effective bandwidth of MPI_Reduce for a set of message sizes.
 *============================================================================*/
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "MPI_Reduce", file1[64], file2[64];
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "reduce_time-np_%.4d.dat", nprocs );
        sprintf( file2, "reduce_bw-np_%.4d.dat",   nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
    ...
    MPI_Reduce( A, B, smed, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD );
    // Test is current NLOOP is enough to capture fastest test cases
    MPI_Barrier( MPI_COMM_WORLD );
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        MPI_Reduce( A, B, smin, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD );
    }
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for(i = 0; i < NREPS; i++){
            ...
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                MPI_Reduce( A, B, size, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD );
            }
