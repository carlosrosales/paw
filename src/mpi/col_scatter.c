 ...
 *==============================================================================
 * Test the effective bandwidth of MPI_Scatter for a set of message sizes.
 *============================================================================*/
...
{
    FILE    *fp, *fp2, *pipe;
    char    testName[32] = "MPI_Scatter", file1[64], file2[64], pipeStr[8];
    ...
    UsedMem = (double)smax*(double)sizeof(double)*( nprocs + 1 );

    // Allocate and initialize arrays
    srand( SEED );
    A = doubleVector( smax*nprocs );
    B = doubleVector( smax );
    ...
    if( proc == 0 ){
        ...
        sprintf( file1, "scatter_time-np_%.4d.dat", nprocs );
        sprintf( file2, "scatter_bw-np_%.4d.dat",   nprocs );
        fp  = fopen( file1, "a" );
        fp2 = fopen( file2, "a" );
    ...
    MPI_Scatter( A, smed, MPI_DOUBLE, B, smed, 
                 MPI_DOUBLE, 0, MPI_COMM_WORLD );
    // Test is current NLOOP is enough to capture fastest test cases
    MPI_Barrier( MPI_COMM_WORLD );
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        MPI_Scatter( A, smin, MPI_DOUBLE, 
                     B, smin, MPI_DOUBLE, 0, MPI_COMM_WORLD );
    }
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        // Warmup with a medium size message
        MPI_Scatter( A, smed, MPI_DOUBLE, 
                     B, smed, MPI_DOUBLE, 0, MPI_COMM_WORLD );
        ...
        for(i = 0; i < NREPS; i++){
            ...
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                MPI_Scatter( A, size, MPI_DOUBLE, 
                             B, size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            }
