...
int main( int argc, char **argv )
{
    ...
    unsigned int MIN_SIZE = MIN_GPU_BLAS_SIZE, MAX_SIZE = MAX_GPU_BLAS_SIZE;
    ...
    if( getenv( "MAX_GPU_BLAS_SIZE" ) != NULL ) MAX_SIZE = atoi( getenv( "MAX_GPU_BLAS_SIZE" ) );
    usedMem = MAX_SIZE*MAX_SIZE*3.0*sizeof(double);
    ...
    cublasDgemm( handle, CUBLAS_OP_N, CUBLAS_OP_N, MED_SIZE, MED_SIZE, MED_SIZE,
    ...
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        cublasDgemm( handle, CUBLAS_OP_N, CUBLAS_OP_N, MIN_SIZE, MIN_SIZE, MIN_SIZE, 
