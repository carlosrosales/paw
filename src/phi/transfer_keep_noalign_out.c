/*==============================================================================
 * Program  : transfer_keep_noalign_out
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_KEEP_NOAL_OUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_keep_noal_time_out.dat", "a" );
    fp2 = fopen( "mic0_keep_noal_bw_out.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0) out( f0_noal : length(smax) ALLOC KEEP )
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_keep_noal_time_out.dat", "a" );
    fp2 = fopen( "mic1_keep_noal_bw_out.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    #pragma offload_transfer target(mic:1) out( f1_noal : length(smax) ALLOC KEEP )
   ...
   //================================================================
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_keep_noal_time_out.dat", "a" );
    fp2 = fopen( "mic0+1_keep_noal_bw_out.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
