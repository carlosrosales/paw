/*==============================================================================
 * Program  : transfer_alloc_align_out
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_ALLOCA_AL_OUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_alloc_al_time_out.dat", "a" );
    fp2 = fopen( "mic0_alloc_al_bw_out.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0) out( f0 : length(smed) ALLOC FREE )
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_alloc_al_time_out.dat", "a" );
    fp2 = fopen( "mic1_alloc_al_bw_out.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    //================================================================
    // Warmup processor with a medium size exchange
    #pragma offload_transfer target(mic:1) out( f1 : length(smed) ALLOC FREE )
   ...
   //================================================================
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_alloc_al_time_out.dat", "a" );
    fp2 = fopen( "mic0+1_alloc_al_bw_out.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
