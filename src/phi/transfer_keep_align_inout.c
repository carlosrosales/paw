/*==============================================================================
 * Program  : transfer_keep_align_inout
 ...
 *==============================================================================
 * Test sustained data transfer rate for CPU-PHI-CPU exchanges for a range 
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_KEEP_AL_INOUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_keep_al_time_inout.dat", "a" );
    fp2 = fopen( "mic0_keep_al_bw_inout.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0)  in( f0 : length(smax) ALLOC KEEP )
    ...
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        #pragma offload_transfer target(mic:0)  in( f0 : length(smin) REUSE KEEP )
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:0)  in( f0 : length(size) REUSE KEEP )
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_keep_al_time_inout.dat", "a" );
    fp2 = fopen( "mic1_keep_al_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    #pragma offload_transfer target(mic:1)  in( f1 : length(smax) ALLOC KEEP )
    #pragma offload_transfer target(mic:1) out( f1 : length(smax) REUSE KEEP )
   ...
   //================================================================
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:1)  in( f1 : length(size) REUSE KEEP )
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_keep_al_time_inout.dat", "a" );
    fp2 = fopen( "mic0+1_keep_al_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );

    // Warmup processor with a medium size exchange
    #pragma offload_transfer target(mic:0)  in( f0 : length(smed) REUSE KEEP )
    #pragma offload_transfer target(mic:0) out( f0 : length(smed) REUSE KEEP )
    #pragma offload_transfer target(mic:1)  in( f1 : length(smed) REUSE KEEP )
