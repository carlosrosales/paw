/*==============================================================================
 * Program  : transfer_alloc_align_inout
 ...
 *==============================================================================
 * Test sustained data transfer rate for CPU-PHI-CPU exchange for a range 
 ...
 * transfer, and the data is aligned to a 64 byte boundary.
 *
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_ALLOCA_AL_INOUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_alloc_al_time_inout.dat", "a" );
    fp2 = fopen( "mic0_alloc_al_bw_inout.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0) in( f0 : length(smed) ALLOC FREE )
    // Test is current NLOOP is enough to capture fastest test cases
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        #pragma offload_transfer target(mic:0) in( f0 : length(smin) ALLOC FREE )
        #pragma offload_transfer target(mic:0) out( f0 : length(smin) ALLOC FREE )
    }
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:0) in( f0 : length(size) ALLOC FREE )
                #pragma offload_transfer target(mic:0) out( f0 : length(size) ALLOC FREE )
            }
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_alloc_al_time_inout.dat", "a" );
    fp2 = fopen( "mic1_alloc_al_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    //================================================================
    // Warmup processor with a medium size exchange
    #pragma offload_transfer target(mic:1) in( f1 : length(smed) ALLOC FREE )
   ...
   //================================================================
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:1) in( f1 : length(size) ALLOC FREE )
                #pragma offload_transfer target(mic:1) out( f1 : length(size) ALLOC FREE )
            }
            // Extra 0.5 factor because we go both ways with the copy
            tElapsed[i] = 0.5*( benchTimer() - tStart );
        }
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_alloc_al_time_inout.dat", "a" );
    fp2 = fopen( "mic0+1_alloc_al_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
