/*==============================================================================
 * Program  : transfer_keep_noalign_inout
 ...
 *==============================================================================
 * Test sustained data transfer rate fof CPU-PHI-CPU exchagnes for a range 
 ...
 * CPU without freeing and reallocating between exchanges. Data is not aligned 
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_KEEP_NOAL_INOUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_keep_noal_time_inout.dat", "a" );
    fp2 = fopen( "mic0_keep_noal_bw_inout.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0) out( f0_noal : length(smax) REUSE KEEP )
    ...
    for(j = 0; j < NLOOP; j++){
        ...
        #pragma offload_transfer target(mic:0) out( f0_noal : length(smin) REUSE KEEP )
    }
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            ...
            for(j = 0; j < NLOOP; j++){
                ...
                #pragma offload_transfer target(mic:0) out( f0_noal : length(size) REUSE KEEP )
            }
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_keep_noal_time_inout.dat", "a" );
    fp2 = fopen( "mic1_keep_noal_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    #pragma offload_transfer target(mic:1) out( f1_noal : length(smax) REUSE KEEP )
   ...
   //================================================================
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            ...
            for(j = 0; j < NLOOP; j++){
                ...
                #pragma offload_transfer target(mic:1) out( f1_noal : length(size) REUSE KEEP )
            }
            // Extra 0.5 factor because we go both ways with the copy
            tElapsed[i] = 0.5*( benchTimer() - tStart );
        }
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_keep_noal_time_inout.dat", "a" );
    fp2 = fopen( "mic0+1_keep_noal_bw_inout.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );

    // Warmup processor with a medium size exchange
    #pragma offload_transfer target(mic:0)  in( f0_noal : length(smed) REUSE KEEP )
    #pragma offload_transfer target(mic:0) out( f0_noal : length(smed) REUSE KEEP )
    #pragma offload_transfer target(mic:1)  in( f1_noal : length(smed) REUSE KEEP )
    #pragma offload_transfer target(mic:1) out( f1_noal : length(smed) REUSE KEEP )
    // Reset innermost loop to safe value and local quantities to defaults
    NLOOP = NLOOP_PHI;
    localSize = smin;
    localMax  = 0.0;
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            ...
            // Extra 0.5 factor because we go both ways with the copy
            tElapsed[i] = 0.25*( benchTimer() - tStart );
        }
        msgBytes = (double)( size*sizeof(double));
        post_process( fp, fp2, threshold_hi, tElapsed, tScale, bwScale, size,
                      msgBytes, msgBytes, &NLOOP, &localMax, &localSize );
