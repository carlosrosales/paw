/*==============================================================================
 * Program  : transfer_alloc_noalign_inout
 ...
 *==============================================================================
 * Test sustained data transfer rate for PHI-CPU-PHI exchanges for a range 
 ...
 *
 * If two Phi coprocessors are present, three total tests are executed, one for 
...
{
    FILE    *fp, *fp2;
    char    testName[32] = "PHI_TRANSFER_ALLOCA_NOAL_INOUT";
    int     micNum, tid;
    ...
    fp  = fopen( "mic0_alloc_noal_time_inout.dat", "a" );
    fp2 = fopen( "mic0_alloc_noal_bw_inout.dat", "a" );
    ...
    #pragma offload_transfer target(mic:0) in( f0_noal : length(smed) ALLOC FREE )
    ...
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
        #pragma offload_transfer target(mic:0) in( f0_noal : length(smin) ALLOC FREE )
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:0) in( f0_noal : length(size) ALLOC FREE )
    ...
    // Open output files and write headers
    fp  = fopen( "mic1_alloc_noal_time_in.dat", "a" );
    fp2 = fopen( "mic1_alloc_noal_bw_in.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
    ...
    //================================================================
    // Warmup processor with a medium size exchange
    #pragma offload_transfer target(mic:1) in( f1_noal : length(smed) ALLOC FREE )
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        for( i = 0; i < NREPS; i++){
            tStart = benchTimer();
            for(j = 0; j < NLOOP; j++){
                #pragma offload_transfer target(mic:1) in( f1_noal : length(size) ALLOC FREE )
    ...
    // Open output files and write headers
    fp  = fopen( "mic0+1_alloc_noal_time_in.dat", "a" );
    fp2 = fopen( "mic0+1_alloc_noal_bw_in.dat", "a" );
    printHeaders( fp, fp2, testName, UsedMem, overhead, threshold_lo );
