 ...
 * using the BLAS DGEMM call for a range of matrix sizes.
 *============================================================================*/
...
int main(int argc, char **argv)
{
    ...
    usedMem  = (double)MAX_SIZE*(double)MAX_SIZE*sizeof(double)*3.0;
    ...
    sprintf( file1, "dgemm_time-np_%.4d.dat", nthreads );
    sprintf( file2, "dgemm_fp-np_%.4d.dat",   nthreads  );
    fp  = fopen( file1, "a" );
    fp2 = fopen( file2, "a" );
    ...
    cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, MED_SIZE, MED_SIZE,
                 MED_SIZE, alpha, A, MED_SIZE, B, MED_SIZE, beta, C, MED_SIZE );
    // Test if current NLOOP is enough to capture fastest test cases
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
         cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, MIN_SIZE, 
                      MIN_SIZE, MIN_SIZE, alpha, A, MIN_SIZE, B, MIN_SIZE, 
                      beta, C, MIN_SIZE );
    }
    ...
    for( size = MIN_SIZE; size <= MAX_SIZE; size = size*2 ){
        ...
        // Warmup processor with a medium size DGEMM
        cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, MED_SIZE, 
                     MED_SIZE, MED_SIZE, alpha, A, MED_SIZE, 
                     B, MED_SIZE, beta, C, MED_SIZE );
