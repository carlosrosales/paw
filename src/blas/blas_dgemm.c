...
int main(int argc, char **argv)
{
    ...
    tStart = benchTimer();
    for(j = 0; j < NLOOP; j++){
         cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, smin, 
                      smin, smin, alpha, A, smin, B, smin, 
                      beta, C, smin );
    }
    ...
    for( size = smin; size <= smax; size = size*2 ){
        ...
        // Warmup processor with a medium size DGEMM
        cblas_dgemm( CblasRowMajor, CblasNoTrans, CblasNoTrans, smed, 
                     smed, smed, alpha, A, smed, 
                     B, smed, beta, C, smed );
