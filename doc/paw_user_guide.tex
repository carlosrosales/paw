...
\vspace{1cm}
Copyright 2015 Carlos Rosales Fern\'andez and the University of Texas at Austin.
\end{flushleft}
\newpage
\end{titlepage}

\begin{abstract}
The Performance Assessment Workbench (PAW) is an Open Source suite of microbenchmarks designed with simplicity and reliability in mind. This User Guide documents the implementation details and the general usage for all components of PAW. As of version 1.5 the suite contains code designed to test:
...
Version 1.5 is a feature release which adds dynamic payload modification to collective tests to avoid running nodes out for memory when executing the benchmarks on large scale systems.
\end{abstract}
...
First of all obtain the latest version tarball, currently paw-1.5.tar.gz, and expand it in a convenient location in your system:

\begin{verbatim}
    tar xzvf ./paw-1.5.tar.gz
\end{verbatim}

This will create a top level directory called \verb+paw-1.5+, with subdirectories \verb+/bin+, \verb+/doc+, and \verb+/src+. Inside the source directory you will also find separate subdirectories for common functions (\verb+aux+) and the floating point (\verb+/blas+), MPI (\verb+/mpi+), GPU (\verb+/gpu+) and Xeon Phi (\verb+/phi+) benchmarking tools.
